...
module(
  ...
  function (hooks) {
    ...
    test('Filter by group name if allowGroupSearch is enabled on PowerSelectMultiple', async function (assert) {
      assert.expect(4);

      this.groupedNumbers = groupedNumbers;

      await render(hbs`
        <PowerSelectMultiple @options={{this.groupedNumbers}} @allowGroupSearch={{true}} @onChange={{action (mut foo)}} @searchEnabled={{true}} as |option|>
          {{option}}
        </PowerSelectMultiple>
      `);
      await clickTrigger();
      ...
      assert.deepEqual(
        groupNames,
        ['Smalls'],
        'Only the groups with matching name are shown'
      );
      assert.deepEqual(
        optionValues,
        ['one', 'two', 'three'],
        'options of the matching group are shown'
      );

      await render(hbs`
        <PowerSelectMultiple @options={{this.groupedNumbers}} @onChange={{action (mut foo)}} @searchEnabled={{true}} as |option|>
          {{option}}
        </PowerSelectMultiple>
      `);
      await clickTrigger();
      ...
      assert.deepEqual(
        groupNames,
        [],
        'filter does not respect group name as allowGroupSearch is not enabled'
      );
      assert.deepEqual(
        optionValues,
        ['No results found'],
    ...
    test('Filter by group name if allowGroupSearch is enabled', async function (assert) {
      ...
      assert.deepEqual(
        groupNames,
        [],
        'filter does not respect group name as allowSearch is not enabled'
      );
      assert.deepEqual(
        optionValues,
        ['No results found'],
